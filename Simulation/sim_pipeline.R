...
# set bounds for body size - min and max of calibration data
bmBounds <- c(0.61, 3.75)
...
tMax <- uniroot(function(t) expTotalNt(t) - nExp, 
                c(meStart + meDur + reDur, meStart + meDur + reDur + 100))$root
...
lambda <- function(t, traits) {
  ...
  ifelse((t < reStart) || (t > reEnd),                    
         ...
         # RE
         lambda_re - lambdaModCont * lambda_re * abs(traits[1] - bmX0))
}
...
mu <- function(t, traits) {
  ifelse((t < meStart) || (t > meStart + meDur),
         # BG
         mu_bg + muModCont * mu_bg * abs(traits[1] - bmX0),
         # ME
...
simulate_rep <- function(tMax, lambda, mu, nFinal, rho, bins,
  ...
  while (!bounds) {
    if (null) {
      ...
      sim <- bd.sim(1, lambda, mu, tMax, nFinal = nFinal, nExtant = c(1, Inf))
    } else {
      # run simulation - want at least one living species at the end
      sim <- bd.sim.traits(1, lambda, mu, tMax, nTraits = 2,
                           traitModel = c("BM", "ST"), pars = pars,
                           nFinal = nFinal, nExtant = c(1, Inf))
  ...
  if (null) {
    res <- list(SIM = sim, SAMP = sample, TREE = tree, SATREE = saTree)
  } else {
    res <- list(SIM = sim, SAMP = sample, TREE = tree, SATREE = saTree,
